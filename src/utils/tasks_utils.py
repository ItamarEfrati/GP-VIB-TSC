...
def task_wrapper(execute_task: Callable) -> Callable:
    ...
    def wrap(config: DictConfig, *args):
        apply_extra_utilities(config)
        n_errors = 0
        metric_dict = {config.get("optimized_metric"): 0}
        while n_errors < 3:
            start_time = time.time()
            try:
                metric_dict = execute_task(config, *args)
                n_errors = 4
            except ValueError:
...
def apply_extra_utilities(config: DictConfig) -> None:
